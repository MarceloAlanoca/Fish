...
@export var nombre_real := "Delfin"
@export var calidad := "Exotico"
@export var vel_progresion := 1.4

var velocidad = Box_Vel.VelP["DelfinVelocity"]
var direccion: Vector2 = Vector2(1, 0)
var distancia_maxima = Box_Vel.Dist["DistDelfin"]
...
func mirar_hacia_direccion():
	if has_node("Sprite2D"):
		if direccion.x < 0:
			...
			$Sprite2D.rotation_degrees = -780   # mirando derecha
		else:
			$Sprite2D.flip_h = direccion.x > 0
			$Sprite2D.rotation_degrees = 780 # mirando izquierda
