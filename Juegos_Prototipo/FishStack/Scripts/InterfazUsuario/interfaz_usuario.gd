...
func _aplicar_efectos_inmediatos():
	...
	if pescador:
		# Solo aplicar efectos de amuletos
		Global.reaplicar_efectos_pescador(pescador, false)

		# Volver a aplicar barco (mantener velocidad correcta)
		if Global.barco_equipado != "":
			Global.aplicar_barco(pescador)

	print("✨ Efectos recalculados (sin reiniciar bases). Amuletos activos:", Global.amuletos_equipados)
